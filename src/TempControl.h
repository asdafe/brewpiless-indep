...
struct ControlVariables{
	...
	temperature posPeak;
	// brewpiless-indep: stale-sensor hold support. While a DS18B20 is briefly
	// off the bus (transient 1-Wire glitch) the independent-mode control loop
	// freezes the cool/heat decision using the last good reading for up to
	// cc.staleSensorHold seconds.
	uint16_t lastValidFridgeTime;    // ticks.seconds() at last good fridgeSensor read
	uint16_t lastValidBeerTime;      // ticks.seconds() at last good beerSensor read
	temperature lastValidFridgeTemp; // last non-disconnected fridge reading
	temperature lastValidBeerTemp;   // last non-disconnected beer reading
};
...
#define MODE_OFF 'o'
#define MODE_TEST 't'
// Independent mode: two independent on/off thermostat loops.
//   * cooling: fridge sensor vs cs.fridgeSetting (cool setpoint)
//   * heating: beer sensor   vs cs.beerSetting   (heat setpoint, may be profile-driven)
// Heat and cool can run simultaneously; no PID, no mutex between them.
...
enum states{
	...
	COOLING_MIN_TIME,			// 8
	HEATING_MIN_TIME,			// 9
	HEAT_AND_COOL,				// 10 — both actuators active simultaneously (INDEPENDENT mode)
	NUM_STATES
};
...
class TempControl{
	...
	// True if the controller is in the new independent (no-PID) mode.
	TEMP_CONTROL_METHOD bool modeIsIndependent(void){
		return cs.mode == MODE_INDEPENDENT;
	}
